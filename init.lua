...
vim.keymap.set('n', '<leader>x', function()
  if not pcall(function() vim.cmd 'bp|bd #' end) then vim.cmd 'enew|bd#' end
...
require('lazy').setup({
  ...
  { -- Fuzzy Finder (files, lsp, etc)
    ...
    config = function()
      ...
      vim.keymap.set('n', '<leader>fw', builtin.live_grep, { desc = '[F]ind using Grep for [w]ords' })
      ...
      vim.keymap.set('n', '<leader>sn', function() builtin.find_files { cwd = vim.fn.stdpath 'config' } end, { desc = '[F]ind [N]eovim files' })
    end,
  },
      ...
      vim.api.nvim_create_autocmd('LspAttach', {
        ...
        callback = function(event)
          ...
          if client and client.server_capabilities.inlayHintProvider and vim.lsp.inlay_hint then
            map('<leader>th', function() vim.lsp.inlay_hint.enable(vim.inspect_pos().buffer, not vim.lsp.inlay_hint.is_enabled()) end, '[T]oggle Inlay [H]ints')
          end
  ...
  },
  {
    ...
    config = function()
      ...
      map('<leader>hm', function() harpoon:list():append() end)
      map('<leader>hn', function() harpoon:list():next() end)
      map('<leader>hp', function() harpoon:list():prev() end)

      map('<leader>ha', function() harpoon.ui:toggle_quick_menu(harpoon:list()) end)
    end,
  },
  {
    'stevearc/oil.nvim',
  ...
  { -- Collection of various small independent plugins/modules
    ...
    config = function()
      ...
      -- Swap some settings and highlight stuff
      -- before:
      -- visible: fg = 838781, bg = 252623
      -- hidden: fg = f1e9d2 bg = 2f312c
      --
      -- change to:
      -- current: fg = f1d9d2 (hidden.fg), bg = 252623 (visible.bg)
      -- visible: fg = f1d9d2 (hidden.fg), bg = 2f312c (hidden.bg)
      -- hidden: fg = 838781 (visible.fg), bg = 2f312c (hidden.bg)
      local old_hidden = vim.api.nvim_get_hl(0, { name = 'MiniTablineHidden' })
      local old_visible = vim.api.nvim_get_hl(0, { name = 'MiniTablineVisible' })
